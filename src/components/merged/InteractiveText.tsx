    ...
    <span className={`inline-block select-none ${className}`}>
      {text.split('').map((char, index) => {
        ...
        return (
          <span
            key={index}
            className={`inline-block transition-all duration-300 transform hover:scale-[1.35] hover:text-[#ffffff] hover:-translate-y-1.5 hover:rotate-[6deg] cursor-default active:scale-90 ${hoverClassName}`}
            style={{
