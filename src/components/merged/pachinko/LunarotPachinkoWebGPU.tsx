...
import { PachinkoWebGPURenderer } from "./PachinkoWebGPURenderer";
...
export default function LunarotPachinkoWebGPU({
  onBallLand,
  onPlanetHit,
  onPachinkoTelemetry,
  beadsCount,
  ...
  const rendererRef = useRef<PachinkoWebGPURenderer | null>(null);
  ...
  useEffect(() => {
    ...
    const initWebGPU = async () => {
      if (gpuCanvasRef.current && !rendererRef.current) {
        rendererRef.current = new PachinkoWebGPURenderer();
        try {
          await rendererRef.current.init(gpuCanvasRef.current);
          resize();
        } catch (e) {
  ...
  const resize = () => {
    ...
    // Bounds calculations to scale dynamically without cutoff
    const isMobile = window.innerWidth < 768;
    const sidebarWidth = isMobile ? 0 : 256;
    let maxW = window.innerWidth - sidebarWidth - 64;
    let maxH = window.innerHeight - 120;
    if (maxW < 280) maxW = 280;
    if (maxH < 220) maxH = 220;

    const scaleW = maxW / DESIGN_W;
    const scaleH = maxH / DESIGN_H;
    let scaleFactor = Math.min(scaleW, scaleH);
    
    // Max scale bounds raised to 2.2 to allow full page size
    scaleFactor = Math.min(scaleFactor, 2.2);
    scaleFactor = Math.max(scaleFactor, 0.35);
    scaleFactorRef.current = scaleFactor;

    const computedCSSW = Math.round(DESIGN_W * scaleFactor);
    const computedCSSH = Math.round(DESIGN_H * scaleFactor);

    canvas.style.width = `${computedCSSW}px`;
    canvas.style.height = `${computedCSSH}px`;
    canvas.width = Math.round(computedCSSW * dpr);
    canvas.height = Math.round(computedCSSH * dpr);

    const gpuCanvas = gpuCanvasRef.current;
    if (gpuCanvas) {
      gpuCanvas.style.width = `${computedCSSW}px`;
      gpuCanvas.style.height = `${computedCSSH}px`;
      gpuCanvas.width = canvas.width;
      gpuCanvas.height = canvas.height;
      if (rendererRef.current) {
        rendererRef.current.updateCamera(DESIGN_W, DESIGN_H);
      }
    }
  ...
  const tickPhysics = () => {
    ...
    for (let i = balls.length - 1; i >= 0; i--) {
      ...
      for (const peg of pegsRef.current) {
        ...
        const Math_distSq = dx * dx + dy * dy;
        if (Math_distSq < minDist * minDist && Math_distSq > 0.0001) {
          const dist = Math.sqrt(Math_distSq);
          const nx = dx / dist;
          const ny = dy / dist;
  ...
  const drawBoard = () => {
    ...
    // Render divider gate partitions in 2D
    const usable = playRight - playLeft;
    const gw = usable / gates.length;
    ...
    // Now render pegs, planets, balls, particles on WebGPU
    if (rendererRef.current) {
      const WebGPUdividers: any[] = [];
      for (let i = 0; i <= gates.length; i++) {
        WebGPUdividers.push({
          x: playLeft + i * gw,
          y: playBottom + 20,
  ...
  const scaledH = DESIGN_H * scaleFactorRef.current;

  return (
    <div ref={rootRef} className="w-full flex flex-col items-center select-none bg-transparent" id="game-wrap">
      <div
        id="frame"
      ...
      >
        {/* Skeleton Background inside cabinet */}
        <img 
          src={pachinkoBgSkeleton} 
          alt="Skeleton Backdrop" 
          className="absolute inset-0 w-full h-full object-cover object-center pointer-events-none select-none z-0 skeleton-breathing"
        />

        {/* Eye fire pixel canvases */}
        <div style={{ position: "absolute", left: "28.125%", top: "6.69%", width: "12%", aspectRatio: "14/34", transform: "translate(-50%, -62%)", pointerEvents: "none", zIndex: 2 }}>
          <canvas ref={fireLeftCanvasRef} width={FW} height={FH} style={{ width: "100%", height: "100%", imageRendering: "pixelated", display: "block", filter: "drop-shadow(0 0 3px rgba(255,150,50,0.75)) drop-shadow(0 0 6px rgba(255,100,20,0.45))" }} />
        </div>
        <div style={{ position: "absolute", left: "55.625%", top: "6.76%", width: "12%", aspectRatio: "14/34", transform: "translate(-50%, -62%)", pointerEvents: "none", zIndex: 2 }}>
          <canvas ref={fireRightCanvasRef} width={FW} height={FH} style={{ width: "100%", height: "100%", imageRendering: "pixelated", display: "block", filter: "drop-shadow(0 0 3px rgba(255,150,50,0.75)) drop-shadow(0 0 6px rgba(255,100,20,0.45))" }} />
        </div>
