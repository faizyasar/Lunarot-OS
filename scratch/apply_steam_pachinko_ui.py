              ...
              s.jsxs("div", {
                ...
                children: [
                  ...
                  s.jsxs("div", {
                    ...
                    children: ["🎴", "🔥", "🔮", "💀"].map((icon, i) => s.jsx("div", { key: i, style: { width: "36px", height: "36px", background: "#0b0f14", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "3px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px" }, children: icon }))
                  })
                ]
              })
            ]
...
# Replace component ip
idx_start = content.find("function ip({")
if idx_start != -1:
    idx_end = content.find("const j=()=>{window.open(\"/sacred-pachinko\",", idx_start)
    # Find matching closing bracket for function ip
    # We locate the end of ip component before the next component definition
    idx_next_fn = content.find("function ", idx_start + 20)
    if idx_next_fn == -1:
        idx_next_fn = content.find("const ", idx_start + 20)
        
    print(f"Replacing function ip from {idx_start} to {idx_next_fn}")
    old_ip_code = content[idx_start:idx_next_fn]
    
    content = content[:idx_start] + steam_pachinko_code + "\n" + content[idx_next_fn:]
    
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(content)
    print("[SUCCESS] Successfully updated index.html with Steam-style Pachinko UI & 3D Mouse Parallax!")
else:
    print("[ERROR] function ip({ not found in index.html!")
