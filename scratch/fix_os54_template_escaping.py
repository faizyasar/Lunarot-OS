...
# 1. Copy OS54 HTML directly to index.html
if os.path.exists(os54_path):
    shutil.copyfile(os54_path, index_path)
    print(f"[SUCCESS] Copied {os54_path} directly to index.html")
else:
    print(f"[ERROR] {os54_path} not found")

with open(index_path, "r", encoding="utf-8") as f:
    content = f.read()

# 2. Update vp array with music.index and deka-archive.index under database
...
# 5. Update dev history string `up` safely
...
if up_start != -1:
    up_end = content.find("`,op=`", up_start)
    if up_end != -1:
        content = content[:up_start] + f"up=`{safe_up_js}" + content[up_end:]
        print("[SUCCESS] Replaced up string safely in OS54")

with open(index_path, "w", encoding="utf-8") as f:
    f.write(content)

print("[COMPLETE] fix_os54_template_escaping script finished.")
