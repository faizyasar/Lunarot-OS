...
async def generate_upright_gif_and_webp():
    ...
    async with async_playwright() as p:
        ...
        () => {
            ...
            // Set camera straight at the upright cassette with subtle 3D tilt
            stage._camera.position.set(0, 0.025, 0.155);
            stage._controls.target.set(0, 0, 0);
            stage._controls.update();
            
            stage._renderer.setAnimationLoop(null);
            
            window.__assembly = assembly;
        ...
        print(f"Capturing {num_frames} frames of upright 360-degree rotation...", flush=True)
        for i in range(num_frames):
            ...
            render_frame_js = f"""
            () => {{
                const assembly = window.__assembly;
                const cassette = window.__cassette;
                const stage = window.__stage;
                
                assembly.rotation.y = {angle_rad};
    ...
    # 1. Animated WebP (native full alpha channel support)
    ...
    # 2. Animated GIF with transparent alpha channel
...
if __name__ == '__main__':
    asyncio.run(generate_upright_gif_and_webp())
