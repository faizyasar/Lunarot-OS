...
print("[SUCCESS] Updated public/history.html")


# ==============================================================================
# 3. UPDATE INDEX.HTML MAIN OS VIEW ROUTING
# ==============================================================================
...
if up_match:
    ...
    print("[SUCCESS] Replaced up string in index.html")

# Search for the rendering block in xp()
old_xp_rendering_block = re.search(r'v==="/apps/sacred-draw\.bin".*?v==="/sys/edit-vessel-handshake\.conf"&&g&&s\.jsx\(yp,\{user:g,onReSeed:I\}\)', content, re.DOTALL)

music_and_markdown_jsx = '''v==="/apps/sacred-draw.bin"&&g&&s.jsx(lp,{user:g,onUpdatePlanets:J,onUpdateActivePlanets:Y,onReset:q,isPurging:ce,setIsPurging:V}),
v==="/apps/astral-pachinko.bin"&&g&&s.jsx(ip,{user:g,onUpdatePlanets:J,onUpdateActivePlanets:Y,onReset:q,isPurging:ce,setIsPurging:V}),
v==="/research/sites-log.md"&&s.jsxs("div",{className:"flex-1 flex flex-col h-full w-full bg-black/35 backdrop-blur-[2px] border border-white/40 p-4 sm:p-6 overflow-y-auto font-mono text-justify",children:[rp(nh)]}),
v==="/research/link-web-map.md"&&s.jsxs("div",{className:"flex-1 flex flex-col h-full w-full bg-black/35 backdrop-blur-[2px] border border-white/40 p-4 sm:p-6 overflow-y-auto font-mono text-justify",children:[rp(sp)]}),
v==="/research/dev-history.md"&&s.jsxs("div",{className:"flex-1 flex flex-col h-full w-full bg-black/35 backdrop-blur-[2px] border border-white/40 p-4 sm:p-6 overflow-y-auto font-mono text-justify",children:[rp(up)]}),
v==="/research/deka-archives.md"&&s.jsxs("div",{className:"flex-1 flex flex-col h-full w-full bg-black/35 backdrop-blur-[2px] border border-white/40 p-4 sm:p-6 overflow-y-auto font-mono text-justify",children:[rp(cp)]}),
v==="/research/social-conduit.md"&&s.jsxs("div",{className:"flex-1 flex flex-col h-full w-full bg-black/35 backdrop-blur-[2px] border border-white/40 p-4 sm:p-6 overflow-y-auto font-mono text-justify",children:[rp(op)]}),
v==="/db/tarot-directory.index"&&s.jsx(pp,{onUpdateActivePlanets:Y,onContextChange:K}),
v==="/db/music.index"&&s.jsxs("div",{className:"flex-1 flex flex-col h-full w-full bg-black/35 backdrop-blur-[2px] border border-white/40 relative z-25 group overflow-hidden p-4 sm:p-6 md:p-8 font-mono space-y-6 overflow-y-auto",children:[
...
v==="/db/deka-archive.index"&&s.jsxs("div",{className:"flex-1 flex flex-col h-full w-full bg-black/35 backdrop-blur-[2px] border border-white/40 p-4 sm:p-6 overflow-y-auto font-mono",children:[s.jsx(ai,{})]}),
v==="/sys/show-aesthetic-tokens.exe"&&s.jsx(gp,{}),
v==="/sys/edit-vessel-handshake.conf"&&g&&s.jsx(yp,{user:g,onReSeed:I})'''

if old_xp_rendering_block:
    content = content.replace(old_xp_rendering_block.group(0), music_and_markdown_jsx)
    print("[SUCCESS] Injected perfect file view routing in xp()")
else:
    print("[WARN] Could not match old_xp_rendering_block")

with open(index_path, "w", encoding="utf-8") as f:
    f.write(content)

print("[COMPLETE] apply_flawless_system script finished.")
