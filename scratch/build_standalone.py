...
print("Loaded textures: side, front, back")
...
<title>Compact Cassette (No Cover)</title>
<style>
...
// Studio Lighting
scene.add(new THREE.HemisphereLight(0xffffff, 0xf0eee8, 1.8));
...
const M = {{
  ...
  tape: new THREE.MeshStandardMaterial({{ name: 'magnetic_tape', color: 0x111111, roughness: 0.65 }}),
  hub: new THREE.MeshStandardMaterial({{ name: 'hub_plastic', color: 0xf5f3ee, roughness: 0.4 }}),
  dark: new THREE.MeshStandardMaterial({{ name: 'liner_black', color: 0x555555, roughness: 0.55 }}),
  metal: new THREE.MeshStandardMaterial({{ name: 'steel', color: 0xc8ced6, roughness: 0.25, metalness: 0.4, transparent: true, opacity: 0.9 }}),
...
// Center cassette group at origin (0,0,0)
const root = new THREE.Group();
root.name = 'cassette_root';
root.add(cassette);
scene.add(root);

// Global control for deterministic frame-by-frame rendering
window.__setRotation = function(yRad, xRad = 0.12, zRad = 0) {{
  root.rotation.set(xRad, yRad, zRad);
  renderer.render(scene, camera);
}};

// Auto spin animation loop for browser viewing
let autoSpin = true;
function animate() {{
...
with open('scratch/cassette_standalone.html', 'w', encoding='utf-8') as f:
    f.write(standalone_html)

print("Saved scratch/cassette_standalone.html")
